...
int isDiff(const char* infile, const char* outfile){
  struct stat* inbuf = malloc(sizeof (struct stat));
  struct stat* outbuf = malloc(sizeof (struct stat));
  int ret = 0;
...
int error_function(int infile, int outfile){
  int neg = infile + outfile;
  neg = -1;
  return neg;
}
